...
from LMS_app.models import Enquiry, Admission, course as CourseModel
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(1, 21):
            ...
            enquiry_date = base_date - timedelta(days=(20 - i) * 1.5)
            ...
            if i <= 12:
                ...
                Admission.objects.create(
                    ...
                    fee_paid=15000.00,
                    admission_fee=2000.00,
                    interested_in_nactet='no',           # This field exists
                    class_schedule='offline',            # This field exists
                    # payment_structure does NOT exist → removed!
                )
                created_admissions += 1

        self.stdout.write(
            self.style.SUCCESS(
                f"SUCCESS! Created {created_enquiries} enquiries and {created_admissions} admissions for November 2025!"
            )
        )
